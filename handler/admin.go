...
func DbLog(c echo.Context) error {
	debug := c.Param("debug")
	if debug == "true" {
		model.Gdb.LogMode(true)
		return SimpleResponse(c, http.StatusOK, 0, "db log set true")
	} else if debug == "false" {
		model.Gdb.LogMode(false)
		return SimpleResponse(c, http.StatusOK, 0, "db log set false")
	} else {
		return SimpleResponse(c, http.StatusNotFound, -1, "unknown command")
	}
}

func LogLevel(c echo.Context) error {
	level := c.Param("level")
	if err := g.InitLog(level); err == nil {
		return SimpleResponse(c, http.StatusOK, 0, "log level set")
	} else {
		return DataResponse(c, http.StatusNotFound, -1, "log level error", err)
	}
}

func ReadLog(c echo.Context) error {
	fileStr := c.QueryParam("file")
	file, ferr := os.OpenFile(g.Config().Log+fileStr, os.O_RDONLY, 0644)
	defer file.Close()
	if ferr != nil {
